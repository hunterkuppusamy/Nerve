...
pub fn main() !void {
    ...
    terminal.setState(undefined, .{ .attribute = .reset });
    ...
    for (builtin.test_functions) |t| {
        ...
        terminal.setState(.background, .{ .color = .black });
        Printer.border();
        terminal.setState(.background, .{  .attribute = .background_default });
        Printer.fmt("\n\n> test ", .{});
        terminal.setState(.foreground, .{ .color = .green });
        Printer.fmt("'{s}'", .{ t.name });
        terminal.setState(.foreground, .{ .color = .cyan });
        Printer.fmt(":\n\n", .{});
        terminal.setState(undefined, .{ .attribute = .reset });
        slowest.startTiming();
        const result = t.func();
        ...
        if (env.verbose) {
            ...
            terminal.setState(.foreground, .{ .attribute = .italic });
            terminal.setState(.foreground, .{ .rgb_color = .{ 0x6a, 0x6a, 0x6a } });
            Printer.fmt("\n// test '{s}' complete in {d:.2}ms.\n", .{ t.name, ms });
...
const Printer = struct {
    ...
    fn status(s: Status, comptime format: []const u8, args: anytype) void {
        switch (s) {
            .pass => terminal.setState(.foreground, .{ .rgb_color = .{ 10, 200, 40 } }),
            .fail => terminal.setState(.foreground, .{ .color = .red }),
            .skip => terminal.setState(.foreground, .{ .rgb_color = .{ 100, 100, 100 } }),
            else => {},
        }
        std.debug.print(format, args);
        terminal.setState(undefined, .{ .attribute = .reset });
...
const SlowTracker = struct {
    ...
    fn display(self: *SlowTracker) !void {
        ...
        terminal.setState(.foreground, .{ .color = .yellow });
        Printer.fmt("Slowest {d} test{s}: \n", .{ count, if (count != 1) "s" else "" });
        var i: usize = 1;
        terminal.setState(.foreground, .{ .color = .red });
